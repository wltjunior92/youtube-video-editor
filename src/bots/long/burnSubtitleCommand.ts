...
export function burnSubtitleCommand(
  ...
  return `ffmpeg -y -i "${inputPath}" -vf "ass='${safeAssPath}':fontsdir='${safeFontsDir}':original_size=1920x1080" -c:v libx264 -preset veryfast -crf 18 -pix_fmt yuv420p -c:a copy "${outputPath}"`;
}